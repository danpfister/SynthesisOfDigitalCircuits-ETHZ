; ModuleID = 'test_control_dependency_2.cpp_mem2reg_constprop_simplifycfg.ll'
...
if.end11:                                         ; preds = %if.else7, %if.then
  %c.addr.0 = phi i32 [ %shr2, %if.then ], [ %c, %if.else7 ]
  %a.addr.0 = phi i32 [ %mul, %if.then ], [ %shr10, %if.else7 ]
  %add12 = add nsw i32 %a.addr.0, %b
  %shr13 = ashr i32 %n, %c.addr.0
  %mul14 = mul nsw i32 %add12, %shr13
  br label %return

return:                                           ; preds = %if.end11, %if.else
  %retval.0 = phi i32 [ %mul14, %if.end11 ], [ %b, %if.else ]
  ret i32 %retval.0
}
